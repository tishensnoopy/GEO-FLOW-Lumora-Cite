...
# --------------------------------------------------------------------------- #
# 进度 total 与实际检测数一致（修复进度 >100% bug）                            #
# --------------------------------------------------------------------------- #

@pytest.mark.asyncio
async def test_check_all_pending_updates_total_to_actual_count(_patch_session, monkeypatch):
    """check_all_pending 应把 task 的 total 更新为实际 pending 数。

    回归：trigger_scan 用第一次查询的 pending 设 create_task(total=N)，
    check_all_pending 内部重新查询 pending，若期间有新分发同步进来，
    实际检测数 M > N，processed 累加到 M > task.total=N → ScanPanel 进度 >100%。
    修复：check_all_pending 开头用实际 pending 数更新 task.total。
    """
    pending = [(f"https://example.com/{i}", f"client-{i}") for i in range(5)]
    checker = _make_checker_with_pending(pending)
    ...
    update_calls = []
    monkeypatch.setattr(
        "app.services.citation_checker.update_progress",
        lambda tid, **kw: update_calls.append((tid, kw)),
    )

    await checker.check_all_pending(task_id="task-total-fix", concurrency=2)

    # 应至少有一次 update_progress 调用包含 total=5（实际 pending 数）
    total_updates = [kw for tid, kw in update_calls if "total" in kw]
    assert len(total_updates) >= 1, "check_all_pending 应更新 task 的 total"
    assert total_updates[0]["total"] == 5, (
        f"total 应更新为实际 pending 数 5，实际 {total_updates[0]['total']}"
    )
